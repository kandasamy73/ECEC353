/* Program implementing the dining philospher synchronization problem 
 * 
 ...
 *
 * Compile as follows: 
 * gcc -o dining_philosophers dining_philosophers.c -std=c99 -Wall -lpthread -lm -lrt
*/
...
{
    pthread_t tid[NUM_PHILOSOPHERS]; /* Thread IDs corresponding to each philosopher */
    ...
    while (curr_num_meals < MAX_NUM_MEALS) {
        ...
        if (chopstick[curr] == 1) { /* Left chop stick is available */
            ...
            if (chopstick[(curr + 1) % NUM_PHILOSOPHERS] == 1) { /* Available */
                ...
                int eat_time = UD (MIN_TIME, MAX_TIME);
                ...
                /* Digest food */
                int digest_time = UD (MIN_TIME, MAX_TIME);
                sleep(digest_time);
            }
